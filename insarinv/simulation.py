...
def get_medfilt_indicies(values):
    n_filter = 3
    n = values.size
    inds_med = np.zeros(n, dtype=int)
    for ii in range(n):
        if ii == 0:
            tmp_values = values[ii:ii+3]
            med_val = np.median(tmp_values)
            inds_med[ii] = np.argwhere(tmp_values == med_val)[0]
        elif ii == n-1:
            tmp_values = values[ii-3:ii]
            med_val = np.median(tmp_values)
            inds_med[ii] = np.argwhere(tmp_values == med_val)[0] + ii - 2
        else:
            tmp_values = values[ii-1:ii+2]
            med_val = np.median(tmp_values)
            inds_med[ii] = np.argwhere(tmp_values == med_val)[0] + ii - 1
    return inds_med
...
class InSARSimulation1D(BaseSimulation):
    ...
    _J_n_equiv = None
    _J_h_aquifer = None
    _J_h_aquifer_diff = None
    _J_h_aquifer_0 = None
    _J_b_aquifer = None
    fd_factor = 0.01
    is_h_min0_bc = False
    use_diff_for_h_aquifer = True
    ...
    def fields(self, m):
        ...
        if self.is_h_min0_bc:
            h_min0 = self.h_aquifer[0]
        else:
        ...
        I = np.r_[np.arange(n_time-1) + 1, np.arange(n_time-1) + 1]
        J = np.r_[np.arange(n_time-1), np.arange(n_time-1) + 1]
        data = np.hstack((-np.ones(n_time-1), np.ones(n_time-1)))
        Pdiff = sp.coo_matrix((data, (I, J)), shape=(n_time,n_time))

        b_v = Cv * Pa @ (Psum @ (Pmin @ utils.mkvc(h) + (emin_0-1) * h_min0))

        # median filter
        inds_med = get_medfilt_indicies(np.diff(b_v))
        I = np.arange(n_time)
        J = np.r_[0, inds_med+1]
        data = np.ones(n_time)
        Pmed = sp.coo_matrix((data, (I, J)), shape=(n_time,n_time))

        b_v = Pcumsum @ Pmed @ Pdiff @ b_v
        ...
        Pf = (Cv *  Pcumsum @ Pmed @ Pdiff @ Pa @ Psum @ Pmin + Ce * Pa @ Psum @ (utils.speye(h.size)-Pmin))
        # Pf = Cv *  Pcumsum @ Pmed @ Pdiff @ Pa @ Psum @ Pmin
    ...
    def get_J_b_equiv(self, m, f=None):
        ...
        if self._J_b_equiv is not None:
            ...
            # self.model = m
            dm = (self.b_equivMap.maps[1].P*m)*0.01
            m_pert = m - self.b_equivMap.maps[1].P.T @ (dm)
            d1 = self.dpred(m_pert)
            m_pert = m + self.b_equivMap.maps[1].P.T @ (dm)
            d2 = self.dpred(m_pert)
    ...
    def get_J_h_min0(self, m, f=None):
        ...
        if self._J_h_min0 is not None:
            ...
            # self.model = m
            dm = (self.h_min0Map.maps[1].P*m)*0.01 + 5.
            m_pert = m - self.h_min0Map.maps[1].P.T @ (dm)
            d1 = self.dpred(m_pert)
            m_pert = m + self.h_min0Map.maps[1].P.T @ (dm)
            d2 = self.dpred(m_pert)
    ...
    def get_J_h_aquifer_0(self, m, f=None):

        if self._J_h_aquifer_0 is not None:
            return self._J_h_aquifer_0
        else:
            self.model = m

            N = self.survey.nD
            M = self.h_aquifer.size

            dm = np.zeros(M)
            dm[0] = 0.001
            # m_pert = m - self.h_aquiferMap.maps[1].P.T @ (dm)
            m_pert = m - (dm)
            d1 = self.dpred(m_pert)
            # m_pert = m + self.h_aquiferMap.maps[1].P.T @ (dm)
            m_pert = m + (dm)
            d2 = self.dpred(m_pert)
            self._J_h_aquifer_0 = (d2-d1) / (2*dm[0])
    ...
    def get_J_h_aquifer(self, m, f=None):
        ...
        if self._J_h_aquifer is not None:
            ...
            # J_h_aquifer_0 = self.get_J_h_aquifer_0(m, f=f)
            ...
            Pk = PT[:self.nz, :]
            # Pk.sum() is for the initial condition
            self._J_h_aquifer[:,0] = self.get_drhsdh_aquifer([], 0).T @ SOL + Pk.sum(axis=0)
            self._J_h_aquifer += PaT.T
            # self._J_h_aquifer[:,0] = J_h_aquifer_0

            return self._J_h_aquifer

    def get_J_h_aquifer_diff(self, m, f=None):
        if self._J_h_aquifer_diff is not None:
            return self._J_h_aquifer_diff
        else:
            print ('Compute J')
            self.model = m
            ...
            N = self.survey.nD
            # Need to expand this
            M = self.h_aquiferMap.nP
            J_h_aquifer_diff = np.zeros((N, M))
            ...
            for ii in range(M):
                ...
                # m_pert = m + self.h_aquiferMap.maps[1].P.T @ (dm)
                d2 = self.dpred(m_pert)
                J_h_aquifer_diff[:,ii] = (d2-d1) / (2*dm[ii])

            self._J_h_aquifer_diff = J_h_aquifer_diff
            return self._J_h_aquifer_diff
    ...
    def Jvec(self, m, v, f=None):
        ...
        if self.h_aquiferMap is not None:
            if self.use_diff_for_h_aquifer:
                Jvec += self.get_J_h_aquifer_diff(m, f=f) @ (v)
            else:
    ...
    def Jtvec(self, m, v, f=None):
        ...
        if self.h_aquiferMap is not None:
            if self.use_diff_for_h_aquifer:
                Jtvec += (self.get_J_h_aquifer_diff(m, f=f).T @ v)
            else:
    ...
    def getJtJdiag(self, m, W=None, f=None):
        ...
        JtJdiag = np.zeros(m.size, dtype=float)

        if self.SskvMap is not None:
            JtJdiag += ((self.SskvDeriv.T @ (self.get_J_Sskv(m, f=f).T @ W))**2).sum(axis=1)
        if self.SskeMap is not None:
            JtJdiag += ((self.SskeDeriv.T @ (self.get_J_Sske(m, f=f).T) @ W)**2).sum(axis=1)
        if self.KvMap is not None:
            JtJdiag += ((self.KvDeriv.T @ (self.get_J_Kv(m, f=f).T) @ W )**2).sum(axis=1)
        if self.h_min0Map is not None:
            JtJdiag += ((self.h_min0Deriv.T @ (self.get_J_h_min0(m, f=f).T) @ W )**2).sum(axis=1)
        if self.n_equivMap is not None:
            JtJdiag += ((self.n_equivDeriv.T @ (self.get_J_n_equiv(m, f=f).T) @ W )**2).sum(axis=1)
        if self.b_equivMap is not None:
            JtJdiag += ((self.b_equivDeriv.T @ (self.get_J_b_equiv(m, f=f).T) @ W )**2).sum(axis=1)
        if self.SskaMap is not None:
            JtJdiag += ((self.SskaDeriv.T @ (self.get_J_Sska(m, f=f).T) @ W )**2).sum(axis=1)
        if self.b_aquiferMap is not None:
            JtJdiag += ((self.b_aquiferDeriv.T @ (self.get_J_b_aquifer(m, f=f).T) @ W )**2).sum(axis=1)
        if self.h_aquiferMap is not None:
            if self.use_diff_for_h_aquifer:
                JtJdiag += (((self.get_J_h_aquifer_diff(m, f=f).T) @ W )**2).sum(axis=1)
            else:
                JtJdiag += ((self.h_aquiferDeriv.T @ (self.get_J_h_aquifer(m, f=f).T) @ W )**2).sum(axis=1)
    ...
    def deleteTheseOnModelUpdate(self):
        """
        matrices to be deleted if the model maps are updated
        """
        toDelete = []
        if self._J_h_aquifer is not None:
            toDelete += ['_J_h_aquifer']
        if self._J_h_aquifer_diff is not None:
            toDelete += ['_J_h_aquifer_diff']
        if self._J_h_aquifer_0 is not None:
            toDelete += ['_J_h_aquifer_0']
        return toDelete
